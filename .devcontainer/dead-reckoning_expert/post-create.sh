...
# The telemetry backend for this level: an OpenTelemetry Collector that every
# component reports spans to, and Jaeger to view the assembled traces.
"$REPO_ROOT/lib/jaeger/init.sh" --version 4.11.1 # https://artifacthub.io/packages/helm/jaegertracing/jaeger
...
echo "✨ Applying the OpenTelemetry collector config (targets Jaeger 4.11.1)"
kubectl apply -f "$CHALLENGE_DIR/platform/otel-collector/config.yaml"
kubectl -n otel rollout restart deployment/collector
kubectl -n otel rollout status deployment/collector --timeout=2m
